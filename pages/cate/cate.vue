		...
		<view class="scroll-container">
			...
			<scroll-view class="left-scroll-view" scroll-y="true" :style="{height: windowHeight+'px'}">
				<block v-for="(item,index) in cateList" :key="index ">
					<view class="left-scroll-view-item" :class="active===index?'active':''" @click="activeChange(index)">{{item.cat_name}}</view>
				</block>
			</scroll-view>
			...
			<scroll-view class="right-scroll-view" scroll-y="true" :style="{height: windowHeight+ 'px'}" :scroll-top="scrollTop">
				<view class="right-scroll-item" v-for="(cateitem,index) in cateItemList" :key="index">
					...
					<view class="cateitem2-list">
						<!-- 商品列表 -->
						<view class="cateitem2-item" v-for="(cateitem2,index2) in cateitem.children" :key="index2" @click="toGoodsList(cateitem2)">
